...
crowdhuman_train_dataset = dict(
    type='RepeatDataset',
    times=10,
    dataset=_base_.crowdhuman_train_dataset)
hiertext_train_dataset = dict(
    type='RepeatDataset',
    times=10,
    dataset=_base_.hiertext_train_dataset)
...
text_datasets_w_size = [
    [_base_.o365v1_dataset, -1], # 0.6M
    [_base_.oiv6_dataset, 0.5], # 1.67M
    [_base_.v3det_dataset, -1], # 0.18M
    [_base_.crowdhuman_train_dataset, -1], # 15k
    [_base_.hiertext_train_dataset, -1], # 8k
    [_base_.bamboo1M_dataset, 0.5], # 1.08M
    [_base_.sa_1b_3m_sampled_dataset, 0.5], # 1.5M
    [flickr30k_dataset, -1], # 30k
    [gqa_dataset, -1], # 46k
    [_base_.BambooCLS_3m_sampled_dataset, 0.2], # 1.56M
    [_base_.cc3m_dataset, 0.2], # 1.32M
]
visual_datasets_w_size = [
    [_base_.o365v1_dataset, -1],
    [_base_.oiv6_dataset, 0.5],
    [crowdhuman_train_dataset, -1],
    [hiertext_train_dataset, -1],
    [_base_.bamboo1M_dataset, 0.5],
    [_base_.sa_1b_3m_sampled_dataset, -1],
]
...
train_dataloader = dict(
    ...
    dataset=dict(
        type='ConcatDataset',
        datasets=[d[0] for d in text_datasets_w_size]))
...
alt_train_dataloader = dict(
    ...
    dataset=dict(
        type='ConcatDataset',
        datasets=[d[0] for d in visual_datasets_w_size]))
