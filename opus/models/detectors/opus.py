...
from ..layers.opus_layer import (
    OPUSTransformerEncoder,
    OPUSHybridEncoder,
    OPUSTransformerDecoder,
)
# VisualPromptGenerator lives in trex_layer but is shared by OPUS and TRex2.
from ..layers.trex_layer import VisualPromptGenerator
...
from ...utils.speed_profiler import OPUSSpeedProfiler, opus_segment
...
class OPUS(DINO):
    """OPUS open-vocabulary detector (text / visual / mixed prompts).

    Built on DINO with a hybrid encoder, visual prompt generator (VPG), and
    optional decoder text cross-attention (``cross_attn_text``).
    """
    ...
    def _init_layers(self) -> None:
        ...
        encoder_type = encoder_cfg.pop('type', 'OPUSTransformerEncoder')
        if encoder_type == 'OPUSTransformerEncoder':
            self.encoder = OPUSTransformerEncoder(**encoder_cfg)
        elif encoder_type == 'OPUSHybridEncoder':
            self.encoder = OPUSHybridEncoder(**encoder_cfg)
        else:
            raise ValueError(f'Unsupported encoder type: {encoder_type}')
        self.decoder = OPUSTransformerDecoder(**self.decoder)
    ...
    def _needs_dec_ica(self) -> bool:
        """Stack ``vpg_aux`` content for ``dec_ica`` head loss (train only)."""
        return self.training and getattr(
            self.bbox_head, 'dec_ica_loss_module', None) is not None
    ...
    def _encode_visual_assemble_vpg(
        ...
        if num_categories == 0:
            return None
        need_vpg_aux = self._needs_dec_ica()
        need_content_aux = need_vpg_aux
        ...
        while cat_start < num_categories:
            ...
            if vtm.any():
                ...
                while slot_start < n_valid:
                    ...
                    content_chunk = visual_query[:, :num_q]
                    visual_queries[b_idx, cat_idx] = cls_emb.to(
                        dtype=visual_queries.dtype)
                    if need_content_aux and content_chunk is not None:
                        if content_stack is None:
                            content_stack = content_chunk.new_zeros(
                                (num_categories, bs, num_q,
                                 content_chunk.size(-1)))
                        content_stack[cat_idx, b_idx] = content_chunk.to(
                            dtype=content_stack.dtype)
                    slot_start += n_chunk
            cat_start = cat_end
        visual_dict = {
        ...
        if need_vpg_aux and content_stack is not None:
            visual_dict['vpg_aux'] = {
                'content': content_stack,
                'mask': prep['target_valid_mask'],
                'selected_gt_inds': prep['selected_gt_inds'],
            }
        return visual_dict
    ...
    def _finish_predict_empty(
        ...
        for data_samples in batch_data_samples:
            data_samples.pred_instances = OPUS.empty_pred_instance(device)
        if rescale:
            for data_samples in batch_data_samples:
    ...
    def _forward(
        ...
        with opus_segment(speed_prof, 'backbone'):
            visual_feats = self.backbone(batch_inputs)
        if self.with_neck:
            with opus_segment(speed_prof, 'neck'):
                visual_feats = self.neck(visual_feats)

        with opus_segment(speed_prof, 'prompt_prep'):
            custom_entities = batch_data_samples[0].get("custom_entities", False)
            text_prompts, text_prompts_all_same, entities = self.get_label_prompts(
                batch_data_samples,
                custom_entities=custom_entities,
        ...
        with opus_segment(speed_prof, 'misc_text_len_check'):
        ...
        if use_lm_text:
            with opus_segment(speed_prof, 'text_encoder'):
                text_dict = self.encode_text(
                    text_prompts,
                    text_prompts_all_same,
                    batch_inputs.device)
        ...
        with opus_segment(speed_prof, 'pre_transformer'):
            encoder_inputs_dict, decoder_inputs_dict = self.pre_transformer(
                visual_feats, batch_data_samples)
        with opus_segment(speed_prof, 'encoder'):
            encoder_outputs_dict = self.forward_encoder(
                **encoder_inputs_dict, text_dict=text_dict)
        if self.use_visual_pre_encoder:
        ...
        if use_split_vpg:
            with opus_segment(speed_prof, 'vpg_prepare'):
                prep = self._encode_visual_prepare_interactive(
                    visual_prompts, mem)
            if prep is None:
                visual_dict = None
            else:
                with opus_segment(speed_prof, 'visual_prompt_generator'):
                    visual_dict = self._encode_visual_assemble_vpg(
                        prep, mem, mem_mask, sp, lsi, vr)
        else:
            with opus_segment(speed_prof, 'vpg'):
                visual_dict = self.encode_visual(
                    visual_prompts=visual_prompts,
                    memory=mem,
        ...
        with opus_segment(speed_prof, 'pre_decoder'):
            tmp_dec_in, head_inputs_dict = self.pre_decoder(
                **encoder_outputs_dict,
                batch_data_samples=batch_data_samples)
            decoder_inputs_dict.update(tmp_dec_in)
        with opus_segment(speed_prof, 'decoder'):
            decoder_outputs_dict = self.forward_decoder(
                **decoder_inputs_dict)
        with opus_segment(speed_prof, 'misc_merge_head_inputs'):
            head_inputs_dict.update(decoder_outputs_dict)
        with opus_segment(speed_prof, 'head'):
