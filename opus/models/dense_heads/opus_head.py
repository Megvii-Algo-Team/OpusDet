...
from ..losses import vpg_losses  # noqa: F401  # register VPGContentContrastiveLoss
...
class OPUSHead(DINOHead):
    """Detection head of OPUS (contrastive cls + VPG align / ICA losses)."""
    ...
    def _init_vpg_loss_modules(self) -> None:
        ...
        if 'vl_align' in cfg:
            ...
            vl.pop('log_scale', None)  # VL align uses ``align_log_scale`` only
            vl.pop('fixed_T', None)
        ...
        self.dec_ica_loss_module = None
        self.cc_query_proj = None
        self.cc_content_proj = None
        if 'dec_ica' in cfg:
            dec = dict(cfg.get('dec_ica') or {})
            dec.setdefault('type', 'VPGContentContrastiveLoss')
            self.dec_ica_loss_module = MODELS.build(dec)
            self.cc_query_proj = nn.Linear(self.embed_dims, self.embed_dims)
            self.cc_content_proj = nn.Linear(self.embed_dims, self.embed_dims)
    ...
    def init_weights(self) -> None:
        ...
        if self.cc_query_proj is not None:
            for m in (self.cc_query_proj, self.cc_content_proj):
    ...
    def loss(self, hidden_states: Tensor, references: List[Tensor],
        ...
        losses = self.loss_by_feat(
            *loss_inputs, hidden_states=hidden_states, vpg_aux=vpg_aux)
    ...
    def loss_by_feat(
        ...
        # Decoder content loss needs hs; omit vpg_aux when hs is absent so
        # RoI enc content is not duplicated on every layer.
        decoder_vpg_aux = vpg_aux if hidden_states is not None else None
        losses_cls, losses_bbox, losses_iou, losses_cc = multi_apply(
        ...
        cc_zero = losses_cls[-1] * 0
        loss_dict['loss_content'] = (
            losses_cc[-1] * layer_weights[-1]
            if losses_cc[-1] is not None else cc_zero)
        num_dec_layer = 0
        for loss_cls_i, loss_bbox_i, loss_iou_i, loss_cc_i in \
            ...
            loss_dict[f'd{num_dec_layer}.loss_content'] = (
                loss_cc_i * w if loss_cc_i is not None else cc_zero)
            num_dec_layer += 1

        if enc_cls_scores is not None:
            enc_loss_cls, enc_losses_bbox, enc_losses_iou, enc_loss_content = \
                self.loss_by_feat_single(
                    enc_cls_scores, enc_bbox_preds,
                    batch_gt_instances=batch_gt_instances,
                    batch_img_metas=batch_img_metas,
                    vpg_aux=vpg_aux)
            w_enc = enc_loss_weight
            loss_dict['enc_loss_cls'] = enc_loss_cls * w_enc
            loss_dict['enc_loss_bbox'] = enc_losses_bbox * w_enc
            loss_dict['enc_loss_iou'] = enc_losses_iou * w_enc
            if enc_loss_content is not None:
                loss_dict['enc_loss_content'] = enc_loss_content * w_enc
    ...
    def _loss_content_from_vpg(
            self,
            vpg_aux: Optional[Dict],
            *,
            hidden_states_layer: Optional[Tensor] = None,
            pos_inds_list: Optional[List[Tensor]] = None,
            pos_gt_inds_list: Optional[List[Tensor]] = None,
    ) -> Optional[Tensor]:
        """Decoder ICA: query vs VPG content (``dec_ica`` only; no enc_ica)."""
        if (not self.training or vpg_aux is None or 'content' not in vpg_aux
                or hidden_states_layer is None):
            return None
        if (self.dec_ica_loss_module is None
                or self.cc_query_proj is None
                or pos_inds_list is None or pos_gt_inds_list is None):
            return None
        content = vpg_aux['content']
        mask = vpg_aux['mask'].bool()
        selected_gt_inds = vpg_aux['selected_gt_inds']
        query_feats: List[Tensor] = []
        content_feats: List[Tensor] = []
        bs = hidden_states_layer.size(0)
        for b in range(bs):
            pos_inds = pos_inds_list[b]
            pos_gt = pos_gt_inds_list[b]
            if pos_inds.numel() == 0:
                continue
            for gt_idx in pos_gt.unique():
                q_inds = pos_inds[pos_gt == gt_idx]
                if q_inds.numel() == 0:
                    continue
                q_feat = hidden_states_layer[b, q_inds].mean(dim=0)
                gt_mask = ((selected_gt_inds[:, b, :] == gt_idx)
                           & mask[:, b, :])
                if not gt_mask.any():
                    continue
                c_feat = content[:, b, :, :][gt_mask].mean(dim=0)
                q_feat = self.cc_query_proj(q_feat)
                c_feat = self.cc_content_proj(c_feat)
                query_feats.append(q_feat)
                content_feats.append(c_feat)
        module = self.dec_ica_loss_module
        if len(query_feats) < module.min_pairs:
            return None
        return module(
            content=torch.stack(content_feats, dim=0),
            query=torch.stack(query_feats, dim=0),
        )
    ...
    def loss_by_feat_single(
        ...
        if isinstance(self.loss_cls, QualityFocalLoss):
            raise NotImplementedError(
                'QualityFocalLoss for OPUSHead is not supported yet.')
        else:
