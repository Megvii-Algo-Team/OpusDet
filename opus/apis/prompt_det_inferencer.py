...
# Adapted for OPUS (local I/O only).
import copy
...
def get_img_bytes(filename):
    """Read image bytes from a local (or mmengine-supported) path."""
    if filename.startswith(('nori://', 's3://')):
        raise NotImplementedError(
            'Remote backends (nori://, s3://) are not supported in the '
            'OPUS open-source release. Please download data locally.')
    return mmengine.fileio.get(filename, backend_args=None)
...
class PromptDetInferencer(BaseInferencer):
    ...
    def _load_weights_to_model(self, model: nn.Module,
        ...
        if checkpoint is not None:
            ...
            if 'dataset_meta' in checkpoint_meta:
                ...
                if isinstance(checkpoint_meta['dataset_meta'], dict):
                    model.dataset_meta = {
                        k.lower(): v
    ...
    def _init_pipeline(self, cfg: ConfigType) -> Compose:
        ...
        if 'meta_keys' in pipeline_cfg[-1]:
            pipeline_cfg[-1]['meta_keys'] = tuple(
                meta_key for meta_key in list(pipeline_cfg[-1]['meta_keys']) + ['mode', 'prompt_path']
                if meta_key != 'img_id')
    ...
    def __call__(
            ...
            prompt_path: Optional[Union[str, dict]] = None,
            **kwargs) -> dict:
        """Call the inferencer.
        ...
        # print(visualize_kwargs, postprocess_kwargs, no_save_vis, no_save_pred)
        ...
        if not isinstance(tokens_positive, list):
            tokens_positive = [tokens_positive] * len(ori_inputs)

        if texts is not None: # ovd mode
        ...
        for i in range(len(ori_inputs)):
            ori_inputs[i]['mode'] = mode
            # OPUS interactive visual mode
            if instances is not None:
                ori_inputs[i]['instances'] = instances[i]
            # OPUS generic visual mode (visual.G pkl)
            if prompt_path is not None:
                ori_inputs[i]['prompt_path'] = prompt_path
    ...
    def visualize(self,
        ...
        for single_input, pred in zip(inputs, preds):
            if isinstance(single_input, str):
                img_bytes = get_img_bytes(single_input)
    ...
    def pred2dict(self,
        ...
        if pred_out_dir == '':
            is_save_pred = False
        # print(data_sample.get('img_path'))
        if is_save_pred and data_sample.get('img_path'):
        ...
        if 'embed_instances' in data_sample:
            embed_instances = data_sample.embed_instances.numpy()
            result['embeddings'] = {
                'label_names': embed_instances.label_names,
                # 'labels':embed_instances.labels.tolist(),
                'embedded':embed_instances.embedded.tolist()
            }
        if is_save_pred:
